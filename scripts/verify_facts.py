...
# ---------- 国家归并（历史政权/多重国籍 -> 现代国家） ----------
# Wikidata 的中文标签常返回繁体（神聖羅馬帝國 / 內萊塔尼亞 / 萊茵邦聯…），
# 而归并表用的是简体，导致历史政权名匹配不上 → 大量假阳性。先做繁→简归一。
TRAD2SIMP = str.maketrans({
    "聖": "圣", "羅": "罗", "國": "国", "內": "内", "萊": "莱", "薩": "萨",
    "奧": "奥", "納": "纳", "聯": "联", "蘭": "兰", "倫": "伦", "華": "华",
    "頓": "顿", "茲": "兹", "烏": "乌", "爾": "尔", "亞": "亚", "義": "义",
    "馬": "马", "島": "岛", "統": "统", "領": "领", "麥": "麦", "諾": "诺",
    "臘": "腊", "麥": "麦", "蘭": "兰", "茲": "兹", "時": "时", "蘭": "兰",
    "達": "达", "維": "维", "亞": "亚", "魯": "鲁", "士": "士",
})

# 历史政权/旧称 -> 现代国家。顺序敏感：长键在前，避免被短键抢先命中。
COUNTRY_TABLE = [
    # 德意志系
    ("神圣罗马", "德国"), ("莱茵邦联", "德国"), ("德意志帝国", "德国"), ("普鲁士", "德国"),
    ("符腾堡", "德国"), ("汉诺威", "德国"), ("巴伐利", "德国"), ("萨克森", "德国"),
    ("纳粹", "德国"), ("魏玛", "德国"), ("德意志", "德国"), ("德国", "德国"),
    # 奥地利系
    ("内莱塔尼亚", "奥地利"), ("奥地利帝国", "奥地利"), ("奥匈", "奥地利"),
    ("哈布斯堡", "奥地利"), ("奥地利", "奥地利"),
    # 意大利系
    # 注意译名差异：萨丁尼亚 / 撒丁尼亚 是同一政权(Kingdom of Sardinia)的两种译法
    ("佛罗伦萨", "意大利"), ("托斯卡纳", "意大利"), ("撒丁", "意大利"), ("萨丁", "意大利"),
    ("两西西里", "意大利"), ("西西里", "意大利"), ("威尼斯", "意大利"),
    ("教皇国", "意大利"), ("意大利", "意大利"),
    # 英法俄美中
    ("大不列颠", "英国"), ("联合王国", "英国"), ("英格兰", "英国"), ("苏格兰", "英国"),
    ("英国", "英国"),
    ("法兰西", "法国"), ("法国", "法国"),
    ("苏联", "俄罗斯"), ("俄罗斯帝国", "俄罗斯"), ("沙俄", "俄罗斯"), ("俄罗斯", "俄罗斯"),
    ("美利坚", "美国"), ("美国", "美国"),
    ("清朝", "中国"), ("中华民国", "中国"), ("中国", "中国"),
    # 其他
    ("瑞士", "瑞士"), ("瑞典", "瑞典"), ("挪威", "挪威"), ("丹麦", "丹麦"),
    ("荷兰", "荷兰"), ("比利时", "比利时"), ("西班牙", "西班牙"), ("葡萄牙", "葡萄牙"),
    ("希腊", "希腊"), ("波兰", "波兰"), ("匈牙利", "匈牙利"), ("捷克", "捷克"),
    ("塞尔维亚", "塞尔维亚"), ("克罗地亚", "克罗地亚"), ("罗马尼亚", "罗马尼亚"),
    ("保加利亚", "保加利亚"), ("奥斯曼", "土耳其"), ("土耳其", "土耳其"),
    ("乌克兰", "乌克兰"), ("芬兰", "芬兰"), ("印度", "印度"), ("以色列", "以色列"),
    ("加拿大", "加拿大"), ("澳大利亚", "澳大利亚"), ("新西兰", "新西兰"), ("日本", "日本"),
]
...
def modern_of(label: str) -> str:
    ...
    if not label:
        return ""
    s = label.translate(TRAD2SIMP)
    for key, val in COUNTRY_TABLE:
        if key in s:
            return val
    return s  # 兜底：原样返回
...
def country_consistent(data_country: str, wd_labels: list) -> bool:
    ...
    档案侧可能是多重国籍（"塞尔维亚 / 美国"），必须逐个部分都比，
    否则特斯拉这类"Wikidata 只命中第二国籍"的情况会被误报。
    """
    parts = [p.strip() for p in re.split(r"[／/（(]", data_country) if p.strip()]
    targets = {modern_of(p) for p in parts}
    for lab in wd_labels:
        m = modern_of(lab)
        if m in targets:
            return True
        # 直接包含也算一致（两边都是国家名时）
        if lab and any(lab in p or p in lab for p in parts):
            return True
    return False
